            ...
            [this]
    {
        for (;;)
        {
            ...
            {//大括号作用：临时变量的生存期，即控制lock的时间
                ...
                this->condition.wait(lock,[this] { return this->stop || !this->tasks.empty(); }); //当stop==false&&tasks.empty(),该线程被阻塞 !this->stop&&this->tasks.empty()
...
template<class F, class... Args>
auto ThreadPool::enqueue(F&& f, Args&&... args)  //参数的右值引用，  此处表示参数传入一个函数
